"""Evaluate BASELINE (pre-fine-tuning) model's legal move accuracy with/without '+' in context."""
import os
import sys
import torch as t
import torch
...
# Load BASELINE model (before fine-tuning)
print('Loading BASELINE model (pre-fine-tuning)...')
ckpt_path = '/root/train_ChessGPT/out-chess-8layer/weights.pt'  # Original weights
...
print(f'\n{"="*50}')
print(f'BASELINE RESULTS SUMMARY ({total} samples)')
print(f'{"="*50}')
print(f'\nWITH "+" in context:')
